...
def test_multi_session():
    ...
    for process, port, screen_num in sessions:
        ...
        if poll_result is None:
            ...
            try:
                ...
                if result == 0:
                    print(f"✅ Session {port} (screen {screen_num}): Running and accepting connections")
                    active_sessions.append((process, port, screen_num))
                else:
                    print(f"⚠️ Session {port} (screen {screen_num}): Running but not accepting connections yet")
                    active_sessions.append((process, port, screen_num))
            except Exception as e:
                print(f"❌ Session {port} (screen {screen_num}): Connection test failed - {e}")
        else:
            print(f"❌ Session {port} (screen {screen_num}): Process exited with code {poll_result}")
    ...
    success = len(active_sessions) >= 2  # Consider success if at least 2 sessions worked
    return success
