...
def test_fixed_behavior():
    ...
    print(f"📊 Locally tracked sessions: {len(FirefoxLauncherHandler._active_sessions)}")
    ...
    # Check if handlers are the default ones (not bound to session_registry)
...
def test_session_isolation():
    ...
    class MockHandler1(FirefoxLauncherHandler):
        def __init__(self):
            self.log = type('MockLog', (), {'info': print, 'debug': print, 'warning': print, 'error': print})()
    ...
    print(f"📊 Shared session tracking: {len(FirefoxLauncherHandler._active_sessions)} sessions")
...
if __name__ == "__main__":
    ...
    print(f"- Global signal handlers: DISABLED ✅")
    print(f"- Cross-session killing: PREVENTED ✅")
    print(f"- Multi-session support: MAINTAINED ✅")
