...
def test_xpra_basic():
    ...
    try:
        result = subprocess.run(
            ["xpra", "--version"], capture_output=True, text=True, timeout=10
        )
...
def test_xvfb():
    ...
    try:
        result = subprocess.run(
            ["which", "Xvfb"], capture_output=True, text=True, timeout=5
        )
...
def test_firefox_startup_script():
    ...
    try:
        result = subprocess.run(
            ["which", "firefox-xstartup"], capture_output=True, text=True, timeout=5
        )
        if result.returncode == 0:
...
def test_xpra_with_firefox():
    ...
    session_dir = (
        Path.home() / ".firefox-launcher" / "debug-sessions" / f"firefox-debug-{port}"
    )
    session_dir.mkdir(parents=True, exist_ok=True)
...
def main():
    ...
    results.append(
        ("Firefox startup script", test_firefox_startup_script() is not None)
    )
