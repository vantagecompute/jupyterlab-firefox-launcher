...
"""
from .firefox_handler import FirefoxLauncherHandler, FirefoxCleanupHandler, get_server_proxy_config
...
def _load_jupyter_server_extension(serverapp):
    ...
    firefox_launcher_pattern = _url_path_join(base_url, "firefox-launcher", "api", "firefox")
    
    # Handler for Firefox cleanup API
    firefox_cleanup_pattern = _url_path_join(base_url, "firefox-launcher", "api", "cleanup")
    ...
    try:
        ...
        serverapp.log.info("jupyter-server-proxy detected, registering Firefox proxy")
        ...
        # Method 2: Check for ServerProxyApp in extensions
            ...
            try:
                ...
                for ext_name, ext_instance in getattr(serverapp.extension_manager, 'extensions', {}).items():
                    if 'server_proxy' in ext_name.lower():
                        if hasattr(ext_instance, 'proxy_config'):
                            ext_instance.proxy_config.update(proxy_config)
                            registered = True
                            serverapp.log.info(f"✅ Registered Firefox proxy via extension {ext_name}")
                            break
        ...
        if not registered:
            try:
                ...
                registered = True
                serverapp.log.info("✅ Registered Firefox proxy via jupyter_server_proxy.api")
            except (ImportError, AttributeError) as api_error:
                serverapp.log.debug(f"API registration failed: {api_error}")
        
        if not registered:
            serverapp.log.warning("⚠️ Could not register with jupyter-server-proxy: No compatible registration method found")
            serverapp.log.warning("   Firefox launcher will use direct API endpoints instead of proxy")
            
    except ImportError:
        serverapp.log.warning("jupyter-server-proxy not available, using direct API endpoints")
        
    except Exception as proxy_error:
        serverapp.log.error(f"❌ Error configuring proxy: {type(proxy_error).__name__}: {proxy_error}")
