...
def _cleanup_firefox_profile(port: int) -> bool:
    ...
    try:
        ...
        _logger.error(f"❌ Error cleaning up Firefox profile for port {port}: {type(e).__name__}: {str(e)}")
...
def _create_xpra_command(port: int) -> List[str]:
    ...
    _logger.info(f"✓ Session directory structure ready:")
    ...
    if not firefox_wrapper:
        _logger.debug("   firefox-xstartup not found in PATH, trying development location...")
        ...
        if not firefox_wrapper.exists():
            error_msg = (
                "firefox-xstartup script not found. Please ensure the package is properly installed."
            )
            _logger.error(f"❌ CRITICAL: firefox-xstartup script not found!")
            _logger.error(f"   Checked in PATH: {os.environ.get('PATH', 'N/A')}")
            _logger.error(f"   Checked development location: {firefox_wrapper}")
            raise RuntimeError(error_msg)
        _logger.info(f"✅ Found firefox-xstartup at development location: {firefox_wrapper}")
    else:
    ...
    if not os.access(firefox_wrapper, os.X_OK):
        _logger.error(f"❌ firefox-xstartup script is not executable: {firefox_wrapper}")
        try:
    ...
    if dev_launcher_path := os.getenv("DEV_FIREFOX_LAUNCHER_PATH"):
        _logger.info(f"🔧 Development override: Using firefox wrapper from DEV_FIREFOX_LAUNCHER_PATH={dev_launcher_path}")
    ...
    _logger.info("📋 Xpra configuration: quality={xpra_quality}, compress={xpra_compress}, dpi={xpra_dpi}")
    ...
    try:
        ...
        if 'v3.1' in xpra_version or 'v3.0' in xpra_version:
            _logger.info("⚠️ Xpra v3.x detected - some session warnings are expected but harmless")
            _logger.info("   You may see 'Xsession: unable to launch true' warnings - these can be ignored")
    ...
    _logger.info(f"   HTML5 Client: Enabled (no WebSockets for SlurmSpawner compatibility)")
    _logger.info(f"   Daemon Mode: Disabled (foreground for process management)")
    ...
    _logger.info(f"   Clipboard: Enabled (bidirectional, all content types)")
    _logger.info(f"   Session Isolation: Complete (unified directory structure per session)")
    ...
    _logger.info(f"   Profile Directory: {profile_dir}")
    _logger.info(f"   Temp Directory: {temp_dir}")
    _logger.info(f"   Display Settings: DPI={xpra_dpi}, Quality={xpra_quality}%, Compression={xpra_compress}")
...
class FirefoxLauncherHandler(JupyterHandler):
    ...
    _active_sessions: Dict[Any,Any] = {}  # Dictionary to track {port: process} for multiple sessions
    ...
    async def post(self):
        ...
        try:
            ...
            async with FirefoxLauncherHandler._xpra_startup_lock:
                ...
                self.log.info("🚀 Starting new Firefox session (existing sessions will continue running)")
                ...
                if success and port and process_id:
                    ...
                    self.log.info(f"Firefox launched successfully on port {port} with process ID {process_id}")
                    self.set_status(200)
                    self.write({
                        "status": "success", 
    ...
    async def _start_server_proxy(self):
        ...
        if active_count > 0:
            for session_port, session_info in FirefoxLauncherHandler._active_sessions.items():
                self.log.debug(f"   Port {session_port}: PID {session_info['process_id']}")
        ...
        try:
            ...
            process = subprocess.Popen(
                ...
                env={**os.environ, 'PATH': os.environ.get('PATH', ''), 'XPRA_CRASH_DEBUG': '1'},
                # Use setsid for process group management if available
                preexec_fn=os.setsid if hasattr(os, 'setsid') else None
            )
            
            self.log.info(f"✅ Xpra subprocess created successfully!")
            ...
            for i, wait_time in enumerate(startup_checks):
                await asyncio.sleep(wait_time)  # Use async sleep instead of blocking sleep
                
                try:
                    poll_result = process.poll()
                    if poll_result is None:
                        self.log.debug(f"✓ Startup check {i+1}/{len(startup_checks)}: Process running")
                    else:
                        self.log.error(f"❌ Startup check {i+1}/{len(startup_checks)}: Process exited with code {poll_result}")
                        break
            ...
            try:
                ...
                self.log.error(f"❌ Error checking final process status: {final_poll_error}")
                return False, None, None
                
            if final_poll is None:
                self.log.info(f"🎉 Xpra process startup completed successfully!")
                ...
                self.log.info(f"✅ Session will be tracked independently (multi-session support)")
                ...
                try:
                    ...
                    if result == 0:
                        ...
                        self.log.debug(f"🔍 Port {port} not yet accepting connections (expected during startup)")
                except Exception as port_check_error:
                    self.log.debug(f"🔍 Port check failed (expected during startup): {port_check_error}")
                
                return True, port, process.pid
            else:
                # Process exited immediately - comprehensive error logging
                self.log.error(f"❌ Xpra process startup FAILED!")
            ...
            self.log.error(f"💥 CRITICAL ERROR in _start_server_proxy: {type(e).__name__}: {str(e)}")
    ...
    async def delete(self):
        ...
        try:
            async with FirefoxLauncherHandler._xpra_startup_lock:
                ...
                if stopped:
                    self.set_status(200)
                    self.write({"status": "success", "message": "Firefox stopped successfully"})
                else:
                    self.set_status(200)
                    self.write({"status": "not_running", "message": "Firefox was not running"})
    ...
    async def get(self):
        ...
        try:
            ...
            if not FirefoxLauncherHandler._active_sessions:
                self.set_status(503)
                self.write({"error": "No Firefox sessions are currently running", "status": "no_sessions"})
                return
            ...
            proxy_path = f"/proxy/{port}/"
            self.log.info(f"GET request: Redirecting to Xpra HTML5 interface at {proxy_path}")
    ...
    async def head(self):
        ...
        try:
            ...
            self.log.debug(f"HEAD request: Found {len(FirefoxLauncherHandler._active_sessions)} active sessions, returning 200")
            self.set_status(200)
            self.set_header('Content-Length', '0')
                
        except Exception as e:
            self.log.error(f"HEAD request failed with unexpected error: {type(e).__name__}: {str(e)}")
    ...
    def _stop_firefox(self) -> bool:
        ...
        for port, session_info in sessions_to_stop:
            ...
            try:
                ...
                self.log.info(f"🔥 Terminating managed session: PID {process_id} ({process_name}) on port {port}")
                ...
                try:
                    ...
                    self.log.info(f"✅ Successfully terminated managed session PID {process_id}")
                except psutil.TimeoutExpired:
                    # Force kill if needed
                    self.log.warning(f"⏰ Force killing unresponsive process {process_id}")
                    process.kill()
...
class FirefoxCleanupHandler(JupyterHandler):
    ...
    async def post(self):
        ...
        try:
            ...
            self.log.info(f"🧹 Cleanup request received")
            self.log.info(f"   Process ID: {process_id}")
            self.log.info(f"   Port: {port}")
            self.log.info(f"   Request method: {self.request.method}")
            self.log.info(f"   Content-Type: {self.request.headers.get('Content-Type', 'Not set')}")
            ...
            if process_id == 'all':
                self.log.info("🧹 CLEANUP: Terminating managed Firefox launcher sessions only")
                self.log.info("   (Other Firefox/Xpra processes on the system will NOT be affected)")
                killed_processes = []
                
                # Only clean up our explicitly managed sessions
                sessions_to_clean = list(FirefoxLauncherHandler._active_sessions.items())
                self.log.info(f"   Found {len(sessions_to_clean)} managed sessions to clean up")
                ...
                for session_port, session_info in sessions_to_clean:
                    ...
                    if session_pid:
                        try:
                            ...
                            self.log.info(f"🔥 Terminating managed session: PID {session_pid} ({process_name}) on port {session_port}")
                            ...
                            for child in children:
                                try:
                                    ...
                                    if any(name in child_name for name in ['firefox', 'xpra', 'xvfb']):
                                        child.terminate()
                                        killed_processes.append(f"{child.name()}:{child.pid}")
                                    else:
                                        self.log.debug(f"🤷 Skipping unrelated child process {child.pid}: {child.name()}")
                                except (psutil.NoSuchProcess, psutil.AccessDenied):
                                    pass
                            ...
                            self.log.info(f"⚠️ Managed session PID {session_pid} already terminated")
                        except Exception as session_error:
                            self.log.error(f"❌ Error terminating managed session {session_pid}: {session_error}")
                
                # Nuclear cleanup is OPTIONAL and requires explicit confirmation
                # This is dangerous in multi-session environments!
                nuclear_cleanup = (self.get_argument('nuclear', 'false').lower() == 'true' and 
                                 self.get_argument('confirm_nuclear', 'false').lower() == 'true')
                
                if self.get_argument('nuclear', 'false').lower() == 'true' and not nuclear_cleanup:
                    self.log.warning("💥 Nuclear cleanup requested but not confirmed.")
                    self.log.warning("   Nuclear cleanup will kill ALL Firefox/Xpra processes system-wide!")
                    self.log.warning("   This may interfere with other users or applications!")
                    self.log.warning("   Use ?nuclear=true&confirm_nuclear=true to proceed.")
                
                if nuclear_cleanup:
                    self.log.warning("💥 NUCLEAR CLEANUP: Killing ALL Xpra and Firefox processes on system")
                    self.log.warning("   ⚠️  WARNING: This may affect processes outside this launcher!")
                    ...
                    for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
                        try:
                            ...
                            if 'xpra' in proc_name or 'firefox' in proc_name:
                                ...
                                if any(session_info.get('process_id') == proc.info['pid'] 
                                      for session_info in FirefoxLauncherHandler._active_sessions.values()):
                                    continue
                                    
                                self.log.warning(f"💀 Nuclear cleanup: Killing {proc_name} process {proc.info['pid']}")
                                proc.terminate()
                                killed_processes.append(f"NUCLEAR-{proc_name}:{proc.info['pid']}")
                        except (psutil.NoSuchProcess, psutil.AccessDenied):
                            continue
                else:
                    self.log.info("🔒 Multi-session safe: Nuclear cleanup not requested, other processes protected")
                
                # Clean up session directories only if requested and only for terminated sessions
                cleanup_directories = self.get_argument('cleanup_dirs', 'false').lower() == 'true'
                
                if cleanup_directories:
                    self.log.info("🧹 Directory cleanup requested - cleaning up directories for terminated sessions only")
                    try:
                        home_dir = Path.home()
                        ...
                        if sessions_dir.exists():
                            ...
                            for session_port, session_info in sessions_to_clean:
                                ...
                                if session_dir.exists() and session_dir.is_dir():
                                    try:
                                        self.log.info(f"🗑️ Removing session directory: {session_dir}")
                                        shutil.rmtree(session_dir)
                                        self.log.info(f"✅ Successfully removed session directory for port {session_port}")
                                    except Exception as dir_error:
                                        self.log.warning(f"⚠️ Failed to remove session directory {session_dir}: {dir_error}")
                        else:
                            self.log.debug("📁 Sessions directory does not exist, nothing to clean up")
                        
                    except Exception as cleanup_error:
                        self.log.error(f"❌ Error during directory cleanup: {cleanup_error}")
                else:
                    self.log.debug("📁 Directory cleanup not requested (use ?cleanup_dirs=true to enable)")
                ...
                remaining_sessions = len(FirefoxLauncherHandler._active_sessions)
                self.log.info(f"🗑️ Cleared {len(sessions_to_clean)} terminated sessions from tracking")
                self.log.info(f"🔒 Multi-session safe: {remaining_sessions} other sessions remain active")
                ...
                features_text = f" ({', '.join(cleanup_features)})" if cleanup_features else ""
            ...
            try:
                ...
                children = process.children(recursive=True)
                self.log.info(f"🔍 Found {len(children)} child processes under PID {process_id}")
                ...
                for child in children:
                    try:
                        ...
                        # Only terminate processes that are clearly related to Firefox/Xpra
                        if any(name in child_name for name in ['firefox', 'xpra', 'xvfb']):
                            self.log.info(f"🔥 Terminating child process {child.pid}: {child.name()}")
                            child.terminate()
                        else:
                            self.log.debug(f"🤷 Skipping unrelated child process {child.pid}: {child.name()}")
                    except psutil.NoSuchProcess:
                        self.log.debug(f"⚠️ Child process {child.pid} already terminated")
                    except Exception as child_error:
                        self.log.warning(f"❌ Error terminating child {child.pid}: {child_error}")
                
                # Terminate the main process
                self.log.info(f"🔥 Terminating main process {process_id}: {process_name}")
                process.terminate()
                ...
                try:
                    ...
                    self.log.warning(f"⏰ Process {process_id} didn't terminate gracefully, force killing...")
                    process.kill()
                    try:
                        process.wait(timeout=2)
                        self.log.info(f"💀 Force killed process {process_id}")
                    except psutil.TimeoutExpired:
                        self.log.error(f"❌ Failed to kill process {process_id} even with force")
                
                # Clean up Firefox profile for this process
                self.log.info(f"🧹 Cleaning up Firefox profile for process {process_id}")
                profile_cleanup_success = _cleanup_firefox_profile(process_id)
                if profile_cleanup_success:
                    self.log.info(f"✅ Successfully cleaned up Firefox profile for process {process_id}")
                else:
                    self.log.warning(f"⚠️ Failed to clean up Firefox profile for process {process_id}")
                ...
                if port_to_remove:
                    ...
                    self.log.info(f"🗑️ Removed session from active tracking (port {port_to_remove})")
                ...
                self.log.info(f"🧹 Cleaning up Firefox profile for terminated process {process_id}")
                profile_cleanup_success = _cleanup_firefox_profile(process_id)
                if profile_cleanup_success:
                    self.log.info(f"✅ Successfully cleaned up Firefox profile for process {process_id}")
                else:
                ...
                self.log.debug(f"🔒 Multi-session mode: Skipping stray process cleanup to protect other active sessions")
                killed_processes = []
